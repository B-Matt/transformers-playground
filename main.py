...
class Trainer:
    def __init__(self, gpu_id, args, net, device):
    ...
    def get_loaders(self):
        self.train_dataset = Dataset(data_dir=r'dataset', img_dir=r'imgs', type=DatasetType.TRAIN, patch_size=self.args.patch_size, transform=self.train_transforms)
        self.val_dataset = Dataset(data_dir=r'dataset', img_dir=r'imgs', type=DatasetType.VALIDATION, patch_size=self.args.patch_size, transform=self.val_transforms)
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.args.epochs):
            val_loss = 0.0
            progress_bar = tqdm(total=int(len(self.train_dataset)), desc=f'Epoch {epoch + 1}/{self.args.epochs}', unit='img', position=0, disable=self.gpu_id != 0)
            ...
            for i, batch in enumerate(self.train_loader):
                ...
                with torch.cuda.amp.autocast(enabled=self.args.use_amp):
                    outputs = self.model(pixel_values=batch_image, labels=batch_mask)
                    loss, logits = outputs.loss, outputs.logits
                    upsampled_logits = torch.nn.functional.interpolate(logits, size=batch_mask.shape[-2:], mode="bilinear", align_corners=False)
                    masks_pred = upsampled_logits.argmax(dim=1)
                ...
                if eval_step > 0 and global_step % eval_step == 0:
                    val_loss = validate(self.model, self.val_loader, self.device, self.gpu_id, epoch, wandb_log)
...
def ddp_trainer_main(rank: int, world_size: int, args: Namespace, net: any):
    ...
    try:
        trainer = Trainer(rank, args, net, torch.device(f'cuda:{rank}'))
        trainer.train()
...
if __name__ == '__main__':
    ...
    if args.use_ddp:
        ...
        mp.spawn(ddp_trainer_main, args=(world_size, args, net), nprocs=world_size)
    ...
    if not args.use_ddp:
        ...
        trainer = Trainer(0, args, net, device)
